...
from auth_service.intelligence.vault_intel.domains.chat import ChatSourceDomain
from auth_service.intelligence.vault_intel.domains.document import DocumentSourceDomain
from auth_service.intelligence.vault_intel.domains.linkedin import LinkedInSourceDomain
from auth_service.intelligence.vault_intel.domains.social import SocialSourceDomain
from auth_service.intelligence.vault_intel.types import (
    ExtractionBundle,
    ExtractionRequest,
    SourceKind,
)
...
class VaultIntelligenceService:
    ...
    async def extract(self, request: ExtractionRequest) -> ExtractionBundle:
        ...
        if self._memory and bundle.candidates:
            try:
                summary = (
                    f"VaultIntel[{request.source.value}] extracted "
                    + ", ".join(sorted({c.field_key for c in bundle.candidates}))
                )
                await self._memory.remember(
                    ...
                    metadata={
                        ...
                        "fields": sorted({c.field_key for c in bundle.candidates}),
                    },
                )
            except Exception:
                logger.exception("Vault intel memory note failed")
        return bundle
