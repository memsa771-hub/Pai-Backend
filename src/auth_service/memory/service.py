...
from auth_service.memory.postgres_store import AsyncPostgresMemoryStore, InProcessMemoryStore

logger = logging.getLogger(__name__)


class PersonMemoryService:
    """Facade: AgentSpan ConversationMemory + SemanticMemory per student.

    Agents may *read* memory via tools; writes go through this deterministic service.
    Vault remains the source of truth for structured profile fields — semantic memory
    holds preferences, constraints, and counseling insights that do not belong in Vault.
    """
    ...
    async def recall(self, query: str, *, top_k: int | None = None) -> str:
        ...
        if self._async_store is not None:
            entries = await self._async_store.search(query, top_k=k)
            if not entries:
                return ""
